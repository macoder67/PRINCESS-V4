...
async (conn, mek, m, { reply }) => {
    try {
        const colorNames = [
            ...
            "Gray", "Cyan", "Magenta", "Violet", "Indigo", "Teal", "Lavender", "Turquoise","aqua"
        ];
...
async (conn, mek, m, { reply }) => {
    try {
        ...
        // Get local time in South Africa timezone (Africa/Cape  town)
        const localTime = now.toLocaleTimeString("en-US", { 
            hour: "2-digit", 
            minute: "2-digit", 
            second: "2-digit", 
            hour12: true,
            timeZone: "Africa/Cape town" // Setting Africa time zone explicitly
        });
...
async (conn, mek, m, { reply }) => {
    try {
        ...
        // Get the formatted date (e.g., "Monday, may 15, 2025")
