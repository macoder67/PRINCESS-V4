...
macoder({
    pattern: "macoder",
    alias: "macoder",
    desc: "Interact with ChatGPT using the PRINCESS Api.",
    category: "ai",
    react: "🤖",
    use: ".macoder <your query>",
    filename: __filename,
}, async (conn, mek, m, { from, args, q, reply }) => {
    try {
        // Vérification de l'entrée utilisateur
        if (!q) return reply("⚠️ Please provide a query for Macoder Ai.\n\nExample:\n.macoder What is AI?");
        ...
        if (!response || !response.data || !response.data.result) {
            return reply("❌ No response received from the Macoder API. Please try again later.");
        }
        ...
        // Image AI à envoyer
        const ALIVE_IMG = 'https://files.catbox.moe/73mlk5.jpg'; // Remplacez par l'URL de votre image AI

        // Légende avec des informations formatées
        const formattedInfo = `🤖 *Macoder's Response:*\n\n${gptResponse}`;
...
macoder({
    ...
    try {
        ...
        // AI image to attach
        const AI_IMG = 'https://files.catbox.moe/73mlk5.jpg'; // Replace with a valid image URL
